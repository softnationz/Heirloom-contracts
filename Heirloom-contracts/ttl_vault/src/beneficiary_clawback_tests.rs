...
fn setup_clawback_env() -> (Env, Address, Address, TtlVaultContractClient<'static>) {
    let env = Env::default();
    env.mock_all_auths();
...
fn test_clawback_after_grace_period_rejected() {
    ...
    env.ledger()
        .with_mut(|li| li.timestamp = 1_000 + GRACE_PERIOD_SECONDS + 1);
...
fn test_clawback_without_release_mark_rejected() {
    ...
    assert!(
        result.is_err(),
        "clawback without prior release mark should fail"
    );
}
